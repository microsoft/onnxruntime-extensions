...
def add_pre_post_processing_to_transformers(model_name: str, input_model_file: Path, output_model_file: Path):
    ...
    add_ppp.transformers_and_bert(bert_onnx_model, output_model_file, model_name, vocab_file,add_debug_before_postprocessing=True)
    

def verify_results_for_e2e_model(output_model_file: Path, model_name: str, input_bert_model: Path = None):
    """
    Args:
    ...
    if input_bert_model and input_bert_model.exists():
...
def main():
    ...
    verify_results_for_e2e_model(new_model_path, args.model_type, model_path)
