...
def assemble_pod_package(
    staging_dir: Path, xcframework_dir: Path, public_headers_dir: Path, ios_deployment_target: str
):
    ...
    with open(_repo_dir / ".version.txt", mode="r") as version_file:
        pod_version = version_file.readline().strip()
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="""
        Assembles the files for the pod package in a staging directory.
    ...
    parser.add_argument(
        ...
        required=True,
        help="Pod iOS deployment target.",
    )
