...
class TestAutoTokenizer(unittest.TestCase):
    ...
    def test_t5_tokenizer(self):
        ...
        ids = tokenizer.encode(text, return_tensors="np")
        ort_tok = OrtPyFunction.from_model(
            gen_processing_models(tokenizer, pre_kwargs={})[0])
        actual_ids = ort_tok([text])[0]
    ...
    def test_roberta_base(self):
        ...
        m_tok, m_detok = gen_processing_models(
            tokenizer, pre_kwargs={}, post_kwargs={})

        actual_ids = OrtPyFunction(m_tok)([text])[0]
        np.testing.assert_array_equal(ids, actual_ids)

        self.assertEqual(OrtPyFunction(m_detok)(ids)[
                         0], tokenizer.decode(ids[0]))
    ...
    def test_microsoft_phi(self):
        ...
        ids = tokenizer(code, return_tensors="np", return_attention_mask=True)
    ...
    def test_microsoft_phi4_base(self):
        script_dir = os.path.dirname(os.path.abspath(__file__))
        tokenizer_dir = os.path.join(script_dir, 'data', "phi-4-base")

        tokenizer = AutoTokenizer.from_pretrained(
            tokenizer_dir, torch_dtype="auto")
        code = 'This is a sample Code with some unusual words such as saoneuhaoesuth'
