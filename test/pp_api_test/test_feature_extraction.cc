...
TEST(ExtractorTest, TestWhisperFeatureExtraction) {
  ...
  extError_t err = OrtxLoadAudios(ort_extensions::move_ptr(raw_audios), audio_path, 3);
  ...
  err = OrtxSpeechLogMel(feature_extractor.get(), raw_audios.get(), ort_extensions::move_ptr(result));
  ASSERT_EQ(err, kOrtxOK);

  OrtxObjectPtr<OrtxTensor> tensor;
  err = OrtxTensorResultGetAt(result.get(), 0, ort_extensions::move_ptr(tensor));
  ASSERT_EQ(err, kOrtxOK);
