...
class _BertLMHeadModel(BertLMHeadModel):
    """ Here we wrap a class for ONNX model conversion for BertLMHeadModel
    """
    ...
    def forward(self, input_ids, token_ids, attention_mask):
        result = super(_BertLMHeadModel, self).forward(input_ids,
                                                       token_ids=token_ids,
...
class TestPreprocessing(unittest.TestCase):
    ...
    def test_bert_preprocessing(self):
        cfg = BertConfig(n_layer=3, is_decoder=True)
        bert_m = _BertLMHeadModel(cfg)
        bert_m.eval().to('cpu')
        full_model = ONNXCompose(
            bert_m,
            preprocessors=pnp.PreHuggingFaceBert(vocab_file=get_test_data_file(__file__, 'data', 'bert_basic_cased_vocab.txt')))
