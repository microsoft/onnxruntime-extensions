...
// expect error throw
TEST(utils, test_string_join_empty_array) {
  auto ort_env = std::make_unique<Ort::Env>(ORT_LOGGING_LEVEL_WARNING, "Default");
  ...
  // Expect error: Input 1 is the strings to join, it should have at least 1 element but 0 now.
  EXPECT_ANY_THROW(TestInference(*ort_env, model_path.c_str(), inputs, outputs, GetLibraryPath()));
}