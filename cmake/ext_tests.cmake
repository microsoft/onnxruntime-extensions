if(OCOS_ENABLE_SELECTED_OPLIST)
  ...
  message(
    FATAL_ERROR
      "Due to usage of OCOS_ENABLE_SELECTED_OPLIST excluding operators the tests are unable to be built and run"
  )
endif()
...
if(IOS)
  ...
  set_property(
    SOURCE "${TEST_SRC_DIR}/ios/dummy_testee_main.mm"
...
function(add_test_target)
  set(optional_args)
  set(single_value_args # test target name
      TARGET)
  ...
  if(NOT IOS)
    ...
    target_sources(
      ${ARG_TARGET} PRIVATE ${ARG_TEST_SOURCES}
                            "${TEST_SRC_DIR}/unittest_main/test_main.cc")

    target_link_libraries(${ARG_TARGET} PRIVATE ${ARG_LIBRARIES} gtest gmock)

    set(test_data_destination_root_directory
        ${onnxruntime_extensions_BINARY_DIR})

  else()
    # add a test that can run on iOS (simulator) it requires a dummy testee
    # target

    # create a dummy app to use as the testee target in xctest_add_bundle()
    set(dummy_testee_target ${ARG_TARGET}_dummy_testee)
    add_executable(${dummy_testee_target}
    ...
    set_target_properties(
      ...
      PROPERTIES MACOSX_BUNDLE TRUE
                 ...
                 com.onnxruntime.unittest.dummy_testee
    ...
    target_sources(${ARG_TARGET} PRIVATE ${ARG_TEST_SOURCES}
                                         "${TEST_SRC_DIR}/ios/xcgtest.mm")

    target_link_libraries(${ARG_TARGET} PRIVATE ${ARG_LIBRARIES} gtest gmock)

    set(test_data_destination_root_directory
        $<TARGET_FILE_DIR:${dummy_testee_target}>)
  ...
  foreach(test_data_directory ${ARG_TEST_DATA_DIRECTORIES})
    ...
    if(NOT test_data_directory_filename)
      # There's no filename if the path has a trailing directory separator. In
      # that case, try to get the filename of the parent path.
      cmake_path(GET test_data_directory PARENT_PATH test_data_directory_parent)
      cmake_path(GET test_data_directory_parent FILENAME
                 test_data_directory_filename)
    endif()
    if(NOT test_data_directory_filename)
      message(
        FATAL_ERROR
          "Failed to get filename for test data directory: ${test_data_directory}"
      )
    endif()

    set(destination_directory
        ${test_data_destination_root_directory}/${test_data_directory_filename})

    add_custom_command(
      TARGET ${ARG_TARGET}
      POST_BUILD
      COMMAND ${CMAKE_COMMAND} -E copy_directory ${test_data_directory}
...
if("${ONNXRUNTIME}" STREQUAL "ONNXRUNTIME-NOTFOUND")
  message(
    WARNING
      "The prebuilt onnxruntime library was not found, extensions_test will be skipped."
  )
else()
  block()
  if(NOT IOS)
  ...
  set(extensions_target
      $<IF:$<BOOL:${use_extensions_shared_library}>,extensions_shared,ortcustomops>
  )
  ...
  if(use_extensions_shared_library)
    target_compile_definitions(
      extensions_test
      PUBLIC ORT_EXTENSIONS_UNIT_TEST_USE_EXTENSIONS_SHARED_LIBRARY)
  endif()

  if (OCOS_USE_CUDA)
    target_compile_definitions(extensions_test PUBLIC _USE_CUDA)
  endif()

  # FUTURE: This is required to use the ORT C++ API with delayed init which must
  # be done conditionally using ifdef OCOS_BUILD_SHARED in RegisterCustomOps and
  # where onnxruntime_cxx_api.h is included . --- We have to remove the
  # OCOS_BUILD_SHARED when building the test code. It is used to delay
  # population of the ORT api pointer until RegisterCustomOps is called, but the
  # test code needs to create an ORT env which requires the pointer to exist.
  # set(test_compile_definitions ${OCOS_COMPILE_DEFINITIONS})
  # remove(test_compile_definitions "OCOS_SHARED_LIBRARY")
  # target_compile_definitions(extensions_test PUBLIC
  ...
  if(WIN32)
    ...
    file(GLOB ONNXRUNTIME_LIB_FILES CONFIGURE_DEPENDS
         "${ONNXRUNTIME_LIB_FILEPATTERN}")
    add_custom_command(
      TARGET extensions_test
      POST_BUILD
      COMMAND ${CMAKE_COMMAND} -E copy_if_different ${ONNXRUNTIME_LIB_FILES}
              $<TARGET_FILE_DIR:extensions_test>)
  endif()

  # Copy onnxruntime shared library to known location for easy access, e.g., for
  # adb push to emulator or device.
  if(ANDROID)
    add_custom_command(
      TARGET extensions_test
      POST_BUILD
      COMMAND ${CMAKE_COMMAND} -E copy_if_different ${ONNXRUNTIME}
