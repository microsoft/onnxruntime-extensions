...
class SpeechFeatures {
 public:
  ...
  OrtxStatus TorchSTFT(const ortc::Tensor<float>& x,
                     int64_t n_fft,
                     int64_t hop_length,
                     int64_t win_length,
                     ...
                     bool center,
                     bool return_complex,
                     ortc::Tensor<float>& out) {
  // win_length must equal n_fft in STFTNorm backend
  if (win_length != n_fft) {
    return {kOrtxErrorInvalidArgument, "[TorchSTFT]: win_length must equal n_fft for STFTNorm."};
  }
  if (center) {
  ...
  OrtxStatus STFTNorm(const ortc::Tensor<float>& pcm, ortc::Tensor<float>& stft_norm) {
              return {kOrtxErrorInvalidArgument, "[Stft] jebi seeeeeeee heheheehe"};

    return stft_norm_.Compute(pcm, n_fft_, hop_length_, {fft_win_.data(), fft_win_.size()}, n_fft_, stft_norm);
  }

   OrtxStatus STFTEnergySegmentation(const std::vector<float>& audio,
                                    int sr,
                                    int frame_ms,
                                    int hop_ms,
    ...
    // ================================
    // 1. Frame length & hop length
    // ================================
    const int64_t frame_length = static_cast<int64_t>(sr * frame_ms / 1000);
    const int64_t hop_length   = static_cast<int64_t>(sr * hop_ms   / 1000);

    // ================================
    // 2. Wrap input audio into Ort Tensor
    // ================================
    std::vector<int64_t> shape = {1, static_cast<int64_t>(audio.size())};
    ortc::Tensor<float> pcm(&CppAllocator::Instance());
    float* pcm_data = pcm.Allocate(shape);
    std::memcpy(pcm_data, audio.data(), audio.size() * sizeof(float));

    // ================================
    // 3. Compute STFT magnitude (power)
    // ================================
    ortc::Tensor<float> stft_out(&CppAllocator::Instance());
    OrtxStatus s = TorchSTFT(
        pcm,
        frame_length,             // n_fft
        hop_length,               // hop_length
        frame_length,             // win_length
        {},                       // window (will use hann internally)
        /*center=*/false,
        /*return_complex=*/false, // returns power spectrum
        stft_out);
    ...
    // STFTNorm layout: [1, freq_bins, frames]
    ...
    for (int64_t t = 0; t < n_frames; ++t) {
      ...
      for (int64_t f = 0; f < n_freq; ++f) {
        ...
        sum += val;  // already power because TorchSTFT returns mag^2
      }
      energy[t] = sum;
    }
