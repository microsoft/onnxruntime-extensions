...
// The following code is based on the minja project (https://github.com/google/minja).
// with modifications to be more VS and mobile compiler friendly.
/*
  ...
  private:
    ...
    static void dump_string(const json &primitive, std::ostringstream &out, char string_quote = '\'')
    {
      if (!primitive.is_string())
        throw std::runtime_error("Value is not a string: " + primitive.dump());
  ...
  public:
    ...
    size_t size() const
    {
      ...
      if (is_array())
        return array_->size();
      if (is_string())
        return primitive_.get<std::string>().length();
    ...
    {
      if (!callable_)
        throw std::runtime_error("Value is not callable: " + dump());
    ...
    int64_t to_int() const
    {
      ...
      if (is_boolean())
        return get<bool>() ? 1 : 0;
      if (is_number())
        return static_cast<int64_t>(get<double>());
      if (is_string())
      {
        try
        {
    ...
    Value operator+(const Value &rhs) const
    {
      ...
      else if (is_array() && rhs.is_array())
      {
        ...
        for (const auto &item : *array_)
          res.push_back(item);
        for (const auto &item : *rhs.array_)
          res.push_back(item);
        return res;
      }
      else
...
namespace minja
{
  ...
  public:
    ...
    virtual uint32_t get_identifier() const { return 0; };
    ...
    Value evaluate(const std::shared_ptr<Context> &context) const
    {
      ...
      catch (const std::exception &e)
      {
        ...
        out << e.what();
        if (location.source)
          out << error_location_suffix(*location.source, location.pos);
        throw std::runtime_error(out.str());
      }
    }
  };

#define DEFINE_EXPRESION_ID()                         \
  static constexpr std::uint32_t class_id = __LINE__; \
  uint32_t get_identifier() const override { return class_id; }

  template <typename T>
  inline T expr_cast(Expression *expr)
  {
    if (std::remove_pointer_t<T>::class_id != expr->get_identifier())
      return nullptr;
  ...
  {
    static constexpr Type class_type = Type::Text;
    std::string text;
  ...
  {
    static constexpr Type class_type = Type::Expression;
  ...
  {
    static constexpr Type class_type = Type::If;
  ...
  {
    static constexpr Type class_type = Type::Elif;
    std::shared_ptr<Expression> condition;
  ...
  {
    static constexpr Type class_type = Type::Else;
  ...
  {
    static constexpr Type class_type = Type::EndIf;
  ...
  {
    static constexpr Type class_type = Type::Macro;
  ...
  {
    static constexpr Type class_type = Type::EndMacro;
  ...
  {
    static constexpr Type class_type = Type::Filter;
  ...
  {
    static constexpr Type class_type = Type::EndFilter;
  ...
  {
    static constexpr Type class_type = Type::For;
  ...
  {
    static constexpr Type class_type = Type::EndFor;
  ...
  {
    static constexpr Type class_type = Type::Generation;
  ...
  {
    static constexpr Type class_type = Type::EndGeneration;
  ...
  {
    static constexpr Type class_type = Type::Set;
    std::string ns;
    std::vector<std::string> var_names;
  ...
  {
    static constexpr Type class_type = Type::EndSet;
  ...
  {
    static constexpr Type class_type = Type::Comment;
    std::string text;
  ...
  {
    static constexpr Type class_type = Type::Break;
    LoopControlType control_type;
  ...
  inline TPtr token_cast(TemplateToken *token)
  {
    if (std::remove_pointer_t<TPtr>::class_type != token->type)
      return nullptr;
    return static_cast<TPtr>(token);
  }
  ...
  public:
    ...
    {
      if (!expr)
        throw std::runtime_error("ExpressionNode.expr is null");
  ...
  public:
    ...
    void do_render(std::ostringstream &out, const std::shared_ptr<Context> &context) const override
    {
      for (const auto &branch : cascade)
      {
        ...
        {
          if (!branch.second)
            throw std::runtime_error("IfNode.cascade.second is null");
  ...
  public:
    ...
    void do_render(std::ostringstream &out, const std::shared_ptr<Context> &context) const override
    {
      ...
      if (!iterable)
        throw std::runtime_error("ForNode.iterable is null");
      if (!body)
        throw std::runtime_error("ForNode.body is null");
  ...
  public:
    ...
    {
      if (!name)
        throw std::runtime_error("MacroNode.name is null");
      if (!body)
  ...
  public:
    ...
    {
      if (!filter)
        throw std::runtime_error("FilterNode.filter is null");
      if (!body)
        throw std::runtime_error("FilterNode.body is null");
  ...
  public:
    ...
    {
      if (!value)
        throw std::runtime_error("SetNode.value is null");
      if (!ns.empty())
      {
        if (var_names.size() != 1)
        {
        ...
        if (!ns_value.is_object())
          throw std::runtime_error("Namespace '" + ns + "' is not an object");
  ...
  public:
    ...
    {
      if (!template_value)
        throw std::runtime_error("SetTemplateNode.template_value is null");
  ...
  public:
    ...
    {
      if (!condition)
        throw std::runtime_error("IfExpr.condition is null");
      if (!then_expr)
        throw std::runtime_error("IfExpr.then_expr is null");
  ...
  public:
    ...
    Value do_evaluate(const std::shared_ptr<Context> &context) const override
    {
      ...
      {
        if (!e)
          throw std::runtime_error("Array element is null");
  ...
  public:
    ...
    Value do_evaluate(const std::shared_ptr<Context> &context) const override
    {
      ...
      {
        if (!key)
          throw std::runtime_error("Dict key is null");
        if (!value)
          throw std::runtime_error("Dict value is null");
  ...
  public:
    ...
    {
      if (!base)
        throw std::runtime_error("SubscriptExpr.base is null");
      if (!index)
        throw std::runtime_error("SubscriptExpr.index is null");
      ...
      else
      {
        ...
        {
          if (auto t = expr_cast<VariableExpr *>(base.get()))
          {
  ...
  public:
    ...
    {
      if (!expr)
        throw std::runtime_error("UnaryOpExpr.expr is null");
  ...
  public:
    ...
    {
      if (!left)
        throw std::runtime_error("BinaryOpExpr.left is null");
      if (!right)
        throw std::runtime_error("BinaryOpExpr.right is null");
      ...
      auto do_eval = [&](const Value &l) -> Value
      {
        if (op == Op::Is || op == Op::IsNot)
        {
          ...
          if (!t)
            throw std::runtime_error("Right side of 'is' operator must be a variable");
          ...
          auto eval = [&]()
          {
            ...
            if (name == "boolean")
              return l.is_boolean();
            if (name == "integer")
              return l.is_number_integer();
            if (name == "float")
              return l.is_number_float();
            if (name == "number")
              return l.is_number();
            if (name == "string")
              return l.is_string();
  ...
  public:
    ...
    {
      if (!object)
        throw std::runtime_error("MethodCallExpr.object is null");
      if (!method)
        throw std::runtime_error("MethodCallExpr.method is null");
  ...
  public:
    ...
    {
      if (!object)
        throw std::runtime_error("CallExpr.object is null");
      auto obj = object->evaluate(context);
      if (!obj.is_callable())
      {
  ...
  public:
    ...
    Value do_evaluate(const std::shared_ptr<Context> &context) const override
    {
      ...
      {
        if (!part)
          throw std::runtime_error("FilterExpr.part is null");
        if (first)
        {
          first = false;
        ...
        {
          if (auto ce = expr_cast<CallExpr *>(part.get()))
          {
  ...
  private:
    ...
    {
      if (!template_str)
        throw std::runtime_error("Template string is null");
    ...
    json parseNumber(CharIterator &it, const CharIterator &end)
    {
      ...
      while (it != end)
      {
        ...
        {
          if (hasExponent)
            throw std::runtime_error("Multiple exponents");
          hasExponent = true;
          ++it;
        }
        else
    ...
    std::pair<std::shared_ptr<Expression>, std::shared_ptr<Expression>> parseIfExpression()
    {
      ...
      if (!condition)
        throw std::runtime_error("Expected condition expression");
      ...
      if (!consumeToken(else_tok).empty())
      {
        ...
        if (!else_expr)
          throw std::runtime_error("Expected 'else' expression");
      }
    ...
    std::shared_ptr<Expression> parseLogicalOr()
    {
      ...
      if (!left)
        throw std::runtime_error("Expected left side of 'logical or' expression");
      ...
      while (!consumeToken(or_tok).empty())
      {
        ...
        if (!right)
          throw std::runtime_error("Expected right side of 'or' expression");
    ...
    std::shared_ptr<Expression> parseLogicalNot()
    {
      ...
      if (!consumeToken(not_tok).empty())
      {
        ...
        if (!sub)
          throw std::runtime_error("Expected expression after 'not' keyword");
    ...
    std::shared_ptr<Expression> parseLogicalAnd()
    {
      ...
      if (!left)
        throw std::runtime_error("Expected left side of 'logical and' expression");
      ...
      while (!consumeToken(and_tok).empty())
      {
        ...
        if (!right)
          throw std::runtime_error("Expected right side of 'and' expression");
    ...
    std::shared_ptr<Expression> parseLogicalCompare()
    {
      ...
      if (!left)
        throw std::runtime_error("Expected left side of 'logical compare' expression");
      ...
      while (!(op_str = consumeToken(compare_tok)).empty())
      {
        ...
        if (!right)
          throw std::runtime_error("Expected right side of 'logical compare' expression");
        BinaryOpExpr::Op op;
        if (op_str == "==")
          ...
          throw std::runtime_error("Unknown comparison operator: " + op_str);
    ...
    Expression::Parameters parseParameters()
    {
      ...
      if (consumeToken("(").empty())
        throw std::runtime_error("Expected opening parenthesis in param list");
      ...
      while (it != end)
      {
        ...
        if (!expr)
          throw std::runtime_error("Expected expression in call args");

        if (auto ident = expr_cast<VariableExpr *>(expr.get()))
        {
          if (!consumeToken("=").empty())
          {
            auto value = parseExpression();
            if (!value)
              throw std::runtime_error("Expected expression in for named arg");
    ...
    ArgumentsExpression parseCallArgs()
    {
      consumeSpaces();
      if (consumeToken("(").empty())
        throw std::runtime_error("Expected opening parenthesis in call args");
    ...
    std::shared_ptr<Expression> parseStringConcat()
    {
      ...
      if (!left)
        throw std::runtime_error("Expected left side of 'string concat' expression");
      ...
      if (!consumeToken(concat_tok).empty())
      {
        auto right = parseLogicalAnd();
        if (!right)
          throw std::runtime_error("Expected right side of 'string concat' expression");
    ...
    std::shared_ptr<Expression> parseMathPow()
    {
      ...
      if (!left)
        throw std::runtime_error("Expected left side of 'math pow' expression");

      while (!consumeToken("**").empty())
      {
        auto right = parseMathPlusMinus();
        if (!right)
          throw std::runtime_error("Expected right side of 'math pow' expression");
    ...
    std::shared_ptr<Expression> parseMathPlusMinus()
    {
      ...
      if (!left)
        throw std::runtime_error("Expected left side of 'math plus/minus' expression");
      std::string op_str;
      while (!(op_str = consumeToken(plus_minus_tok)).empty())
      {
        auto right = parseMathMulDiv();
        if (!right)
          throw std::runtime_error("Expected right side of 'math plus/minus' expression");
    ...
    std::shared_ptr<Expression> parseMathMulDiv()
    {
      ...
      if (!left)
        throw std::runtime_error("Expected left side of 'math mul/div' expression");
      ...
      while (!(op_str = consumeToken(mul_div_tok)).empty())
      {
        ...
        if (!right)
          throw std::runtime_error("Expected right side of 'math mul/div' expression");
      ...
      if (!consumeToken("|").empty())
      {
        ...
        if (auto filter = expr_cast<FilterExpr *>(expr.get()))
        {
    ...
    std::shared_ptr<Expression> parseMathUnaryPlusMinus()
    {
      ...
      if (!expr)
        throw std::runtime_error("Expected expr of 'unary plus/minus/expansion' expression");
    ...
    std::shared_ptr<Expression> parseExpansion()
    {
      ...
      if (op_str.empty())
        return expr;
      if (!expr)
        throw std::runtime_error("Expected expr of 'expansion' expression");
    ...
    std::shared_ptr<Expression> parseBracedExpressionOrArray()
    {
      ...
      auto expr = parseExpression();
      if (!expr)
        throw std::runtime_error("Expected expression in braced expression");
      ...
      {
        if (consumeToken(",").empty())
          throw std::runtime_error("Expected comma in tuple");
        auto next = parseExpression();
        if (!next)
          throw std::runtime_error("Expected expression in tuple");
    ...
    std::shared_ptr<Expression> parseArray()
    {
      ...
      if (!first_expr)
        throw std::runtime_error("Expected first expression in array");
      ...
      while (it != end)
      {
        if (!consumeToken(",").empty())
        {
          auto expr = parseExpression();
          if (!expr)
            throw std::runtime_error("Expected expression in array");
    ...
    std::shared_ptr<Expression> parseDictionary()
    {
      ...
      auto parseKeyValuePair = [&]()
      {
        ...
        if (!key)
          throw std::runtime_error("Expected key in dictionary");
        if (consumeToken(":").empty())
          throw std::runtime_error("Expected colon betweek key & value in dictionary");
        auto value = parseExpression();
        if (!value)
          throw std::runtime_error("Expected value in dictionary");
    ...
    std::vector<std::string> parseVarNames()
    {
      ...
      if ((group = consumeTokenGroups(varnames_regex)).empty())
        throw std::runtime_error("Expected variable names");
    ...
    {
      return std::runtime_error("Unexpected " + TemplateToken::typeToString(token.type) + error_location_suffix(*template_str, token.location.pos));
    }
    std::runtime_error unterminated(const TemplateToken &token) const
    {
      return std::runtime_error("Unterminated " + TemplateToken::typeToString(token.type) + error_location_suffix(*template_str, token.location.pos));
    }
          ...
          else if (!(group = consumeTokenGroups(block_open_regex, SpaceHandling::Keep)).empty())
          {
            ...
            auto parseBlockClose = [&]() -> SpaceHandling
            {
              if ((group = consumeTokenGroups(block_close_regex)).empty())
                throw std::runtime_error("Expected closing block tag");
              return parsePostSpace(group[1]);
            };

            if ((keyword = consumeToken(block_keyword_tok)).empty())
              throw std::runtime_error("Expected block keyword");
            ...
            else if (keyword == "elif")
            {
              auto condition = parseExpression();
              if (!condition)
                throw std::runtime_error("Expected condition in elif block");
            ...
            else if (keyword == "for")
            {
              ...
              if (consumeToken(in_tok).empty())
                throw std::runtime_error("Expected 'in' keyword in for block");
              auto iterable = parseExpression(/* allow_if_expr = */ false);
              if (!iterable)
                throw std::runtime_error("Expected iterable in for block");
            ...
            else if (keyword == "set")
            {
              ...
              if (!(group = consumeTokenGroups(namespaced_var_regex)).empty())
              {
                ...
                if (consumeToken("=").empty())
                  throw std::runtime_error("Expected equals sign in set block");

                value = parseExpression();
                if (!value)
                  throw std::runtime_error("Expected value in set block");
              }
              else
              {
            ...
            else if (keyword == "macro")
            {
              ...
              if (!macroname)
                throw std::runtime_error("Expected macro name in macro block");
            ...
            else if (keyword == "filter")
            {
              ...
              if (!filter)
                throw std::runtime_error("Expected expression in filter block");
        ...
        bool fully = false) const
    {
      ...
      while (it != end)
      {
        const auto start = it;
        const auto &token = *(it++);
        if (auto if_token = token_cast<IfTemplateToken *>(token.get()))
        {
          std::vector<std::pair<std::shared_ptr<Expression>, std::shared_ptr<TemplateNode>>> cascade;
          ...
          {
            auto elif_token = token_cast<ElifTemplateToken *>((*(it++)).get());
        ...
        else if (auto for_token = token_cast<ForTemplateToken *>(token.get()))
        {
        ...
        else if (token_cast<GenerationTemplateToken *>(token.get()))
        {
          auto body = parseTemplate(begin, it, end);
        ...
        else if (auto text_token = token_cast<TextTemplateToken *>(token.get()))
        {
          ...
          else if (options.trim_blocks && (it - 1) != begin && !token_cast<ExpressionTemplateToken *>((*(it - 2)).get()))
          {
        ...
        else if (auto expr_token = token_cast<ExpressionTemplateToken *>(token.get()))
        {
          children.emplace_back(std::make_shared<ExpressionNode>(token->location, std::move(expr_token->expr)));
        }
        else if (auto set_token = token_cast<SetTemplateToken *>(token.get()))
        {
          if (set_token->value)
          {
          ...
          else
          {
            ...
            if (!set_token->ns.empty())
              throw std::runtime_error("Namespaced set not supported in set with template value");
            if (set_token->var_names.size() != 1)
              throw std::runtime_error("Structural assignment not supported in set with template value");
        ...
        else if (auto macro_token = token_cast<MacroTemplateToken *>(token.get()))
        {
          auto body = parseTemplate(begin, it, end);
        ...
        else if (auto filter_token = token_cast<FilterTemplateToken *>(token.get()))
        {
          auto body = parseTemplate(begin, it, end);
        ...
        else if (token_cast<CommentTemplateToken *>(token.get()))
        {
          // Ignore comments
        }
        else if (auto ctrl_token = token_cast<LoopControlTemplateToken *>(token.get()))
        {
          children.emplace_back(std::make_shared<LoopControlNode>(token->location, ctrl_token->control_type));
        }
        else if (token_cast<EndForTemplateToken *>(token.get())
              || token_cast<EndSetTemplateToken *>(token.get())
              || token_cast<EndMacroTemplateToken *>(token.get())
              || token_cast<EndFilterTemplateToken *>(token.get())
              || token_cast<EndIfTemplateToken *>(token.get())
              || token_cast<ElseTemplateToken *>(token.get())
              || token_cast<EndGenerationTemplateToken *>(token.get())
              || token_cast<ElifTemplateToken *>(token.get()))
        {
  ...
  inline std::shared_ptr<Context> Context::builtins()
  {
    ...
    globals.set("tojson", simple_function("tojson", {"value", "indent"}, [](const std::shared_ptr<Context> &, Value &args)
                                          { return Value(args.at("value").dump(args.get<int>("indent", -1), /* tojson= */ true)); }));
